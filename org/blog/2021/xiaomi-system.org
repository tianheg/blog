...
sudo chmod 700 /var/lib/mysql # d--------- to drwx------ ref:
...
- day2 重新创建 maven 项目 1_高天贺home，定义一个包
...
对于 =driver-class-name= 曾经的内容是
=com.mysql.jdbc.Driver=（已弃用），新的名称是
...
#+caption: Vue MVVM
[[https://cdn.jsdelivr.net/gh/tianheg/static@main/img/vue-mvvm.svg]]
...
*** 第七天
    ...
    :CUSTOM_ID: 第七天
    :END:
#+caption: front-back-end-mysql-framework
...
通过实现三个需求，熟悉写程序的顺序。第一步，根据 HTML
在数据库中加入数据；第二步，项目初始化；第三步，MyBatis 逆向工程生成
domain 下文件和 Mapper 文件；第四步，写 Service
层接口和实现类；第五步，按照 Vue 语法修改 HTML 文件；第六步，添加 axios
get,then 规则；最后一步，在 controller 中添加相应部分。
