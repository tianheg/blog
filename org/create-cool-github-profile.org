...
午饭前看到一个 issue：[[https://github.com/ruanyf/weekly/issues/1616][[开源自荐] 利用
Github Actions 获取网站的 RSS 数据，并更新到个人主页]]。

标题勾起了我学习的兴趣。自己也想进一步学习 [[/tags/github-actions/][GitHub Actions]] 的使用。
* 添加「年进度」
...
* 添加「最近更新的博客文章」
使用了 [[https://github.com/pypa/pipenv][pipenv]] 这个 Python 开发工作流。在与个人用户名相同的仓库中添加 =Pipfile= ：
...
2.  =on= 何时运行。 =workflow_dispatch= 能让我手动执行工作流， =0 */6 * * *= 是一种表示周期时间的语法，当前含义：每六个小时执行一次（[[https://crontab.guru/][了解更多]]）
3.  =jobs= 它后面就是具体的工作流
