...
这本书没有涉及 Scalable Vector Graphics(SVG)——另一个 HTML5 相关的技术。可阅读由 Kurt Cagle 写的 /SVG Programming/ 。
...
因为人的需求太过丰富。这些属性的官方名称是 /author defined attributes/ 。它在 HTML4 中被引入。
...
更多 CSS color 相关标准，见[[https://www.w3.org/TR/css-color-3/][这里]]。
...
*像素单位的问题* ：像素这个术语一般是指显示设备上可寻址的最小单元——图像的基本元素。CSS 却是另辟蹊径，其像素定义如下：参考像素是距读者一臂之遥的像素密度为 96dpi 的设备上一个像素的视角（visualangle）。CSS 中的 px [[https://drafts.csswg.org/css-values-3/#px][标准定义]]。
...
[[https://www.ruanyifeng.com/blog/2014/03/undefined-vs-null.html][undefined与 null 的区别 - 阮一峰的网络日志]]
...
见[[https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/BigInt][MDN]]
