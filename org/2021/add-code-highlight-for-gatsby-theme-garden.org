...
官方仓库里找，终于我找到 =gatsby-plugin-theme-ui=。于是，在
=gatsby-config.js= 设置：
...
一开始代码过长的话，是可以自动折叠的，即通过可能是 =overflow: auto;=
...
当设置 =prismPreset= 为 =prism-twilight=时，和
=gatsby-plugin-local-search= 是矛盾的，无法执行
