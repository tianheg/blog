...
date: 2024-06-26T20:26:00+08:00
tags: ['技术', English]
---
...
I checked the Git commits and reverted them one by one until I successfully resolved the issue. The problem stemmed from my attempt to implement view transitions using "@view-transition", inspired by an Astro blog post titled ["Zero-JavaScript View Transitions"](https://astro.build/blog/future-of-astro-zero-js-view-transitions/), which is not supported in my current browser.
...
Uncertain about how to debug this problem, I sought help from [@luojiyin1987](https://github.com/luojiyin1987). Together, we inserted `console.log(data)`, which successfully produced the desired outcome during local testing. However, upon deployment on Cloudflare, an unexpected `Response()` object was returned:
