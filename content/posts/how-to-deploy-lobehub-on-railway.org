...
I'm not going to remove the official cloud-sandbox -- it's too much trouble. I deployed [[https://github.com/agent-infra/sandbox][AIO sandbox]] on Railway, accessing it through MCP(streamable HTTP). But it needs a lot of memory to run, so I need to enable serverless mode to reduce resource usage. And it takes about 20 minutes to deploy, so don't worry if it doesn't deploy quickly.
