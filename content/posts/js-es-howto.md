...
进一步阅读：[5.2 Algorithm Conventions](https://tc39.es/ecma262/#sec-algorithm-conventions)
...
进一步阅读：[The List and Record Specification Types](https://tc39.es/ecma262/#sec-list-and-record-specification-type)
...
进一步阅读：[The Completion Record Specification Type](https://tc39.es/ecma262/#sec-completion-record-specification-type)
...
进一步阅读：[ReturnIfAbrupt Shorthands](https://tc39.es/ecma262/#sec-returnifabrupt-shorthands)

### JavaScript 对象

规范中，每个对象都有一些内部方法，以使规范的其他部分能够调用。这些内部方法有：
...
详细列表在：[Object Internal Methods and Internal Slots](https://tc39.es/ecma262/#sec-object-internal-methods-and-internal-slots)
...
ECMAScript 规范还允许其他规范定义自己的外来对象。正是通过这种机制，浏览器能指定对跨源 API 访问的限制（参见 [WindowProxy](https://html.spec.whatwg.org/multipage/nav-history-apis.html#windowproxy)）。JavaScript 程序员也可以通过 Proxy API 创建自己的外来对象。
...
在规范左上方的搜索栏搜索“substring”，找到 [String.prototype.substring(start, end)](https://tc39.es/ecma262/#sec-string.prototype.substring)。先想想已知什么：知道 substring 是如何工作的（返回输入字符串的一部分）。不知道的是：当 this 值为 undefined 时，结果如何。所以，在接下来的算法步骤阅读过程中，会专注于思考有关 this 值的部分。
...
- [怎样阅读 ECMAScript 规范？](https://github.com/Pines-Cheng/blog/issues/63)
