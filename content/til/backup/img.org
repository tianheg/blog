...
使用 [[https://play.google.com/store/apps/details?id=dk.tacit.android.foldersync.lite][FolderSync App]]，配置 SFTP 连接到 StorageBox（端口 22），定时任务自动同步 DCIM/Camera 目录下的照片到 StorageBox 根目录。
