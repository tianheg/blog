#+TITLE: X Content Type Options
#+STATUS: draft
#+DATE: 2025-06-15T19:22:54
#+HEADER: Tools
1. `The resource from "http://localhost:1237/_{assets}/js/prism/prism-sh.min.js" was blocked due to MIME type ("text/plain") mismatch (X-Content-Type-Options: nosniff).`

[[https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/X-Content-Type-Options][X-Content-Type-Options - HTTP | MDN]]

当 `X-Content-Type-Options` 的值为 nosniff 时，如果一个样式文件的 `Content-Type` 不是 `text/css`，或者一个脚本文件的 `Content-Type` 不是 [[https://mimesniff.spec.whatwg.org/#javascript-mime-type][`text/javascript`]]，那么加载当前文件的请求就会被阻挡，当前文件也就无法发挥作用。
