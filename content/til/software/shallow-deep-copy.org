...
对于可以序列化的对象，还有一个可用的属性方法是 `structuredClone()`，`structuredClone()` has the advantage of allowing [[https://developer.mozilla.org/en-US/docs/Glossary/Transferable_objects][transferable objects]] in the source to be _transferred_ to the new copy。记住，`structuredClone()` 并非 JS 语言的特性，而是一种浏览器和其他 JS 运行时（实现了像 `window` 这样的全局对象）的特性。
...
使用 [[https://developer.mozilla.org/en-US/docs/Web/API/structuredClone][`structuredClone()`]]。对于 Node.js 在 v17.0.0 版本引入。[[https://developer.mozilla.org/en-US/docs/Web/API/structuredClone#browser_compatibility][兼容性一览]]

--------------

参考资料

1. [[https://developer.mozilla.org/en-US/docs/Glossary/Shallow_copy][Shallow copy - MDN Web Docs Glossary: Definitions of Web-related terms | MDN]]
2. [[https://developer.mozilla.org/en-US/docs/Glossary/Deep_copy][Deep copy - MDN Web Docs Glossary: Definitions of Web-related terms | MDN]]
3. [[https://github.com/mqyqingfeng/Blog/issues/32][JavaScript 专题之深浅拷贝 · Issue #32 · mqyqingfeng/Blog]]
