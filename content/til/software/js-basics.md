...
根据标准，64 位浮点数的指数部分的长度是 11 个二进制位，意味着指数部分的最大值是 2047（ \$2<sup>11</sup>$ -1）。也就是说，64 位浮点数的指数部分的值最大为 2047，分出一半表示负数，则 JS 能够表示的数值范围为 \$2<sup>1024</sup>$ 到 \$2<sup>-1024</sup>$ （开区间），超出这个范围的数无法表示。
