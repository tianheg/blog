---
title: 'JS Array Element to Func True'
status: draft
...
```js
function findElement(arr, func) { return arr[arr.map(func).indexOf(true)] }
...
```js
function findElement(arr, func) { return arr.length && !func(arr[0]) ? findElement(arr.slice(1), func) : arr[0] }
