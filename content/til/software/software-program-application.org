#+TITLE: Software, Program 和 Application 的区别
:PROPERTIES:
:CUSTOM_ID: software-program-application
:END:
...
App 是为用户设计带 UI 的软件。Software 比 Program 包含更多东西。
...
这个术语通常指的是为终端用户设计的软件，可以直接在计算机、移动设备或其他电子设备上运行。App 通常具有用户界面，可以是图形界面，也可以是命令行界面。App 通常是为特定目的或一组相关任务设计的，比如电子邮件客户端、游戏、办公软件等。在移动设备上，"app" 通常指的是在操作系统上安装并运行的应用程序，如 iOS 或 Android 上的应用程序。
